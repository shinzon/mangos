...
void IRCCmd::PM_Player(_CDATA *CD)
{
    ...
    if (Player* plr = GetPlayer(_PARAMS[0]))
    {
        ...
        {
            std::string sMsg = MakeMsg("|cffFE87FD[<IRC>%s] (use .ircpm NAME to reply) Whispers: %s|r", CD->USER.c_str(), _PARAMS[1].c_str());
...
void IRCCmd::Tele_Player(_CDATA *CD)
{
    ...
    if(DoTeleport)
    {
        ...
        {
            if(!sIRC.BeenToGMI(pX, pY, pZ, _PARAMS[0], CD->USER))
			{
